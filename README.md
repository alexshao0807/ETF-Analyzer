...
一個 Windows 桌面應用程式，專為投資人與分析師設計。透過自動化演算法，在秒級內比對 ETF 兩日成分股的權重與持股變化，解決人工比對耗時且易錯的痛點。
...
# v2.0 更新內容
1. **新增檔案拖放 (Drag & Drop)**：現在可以直接將 Excel 檔案拖入視窗讀取，操作更直覺。
2. **新增日誌系統 (Log)**：程式執行過程與錯誤訊息會自動記錄於 `Logs` 資料夾，方便除錯與追蹤。
## 👤 作者 (Author)
